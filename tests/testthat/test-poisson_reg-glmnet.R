...
test_that("glmnet multi_predict(): type numeric", {
  ...
  f_pred <- multi_predict(f_fit, seniors, type = "numeric",
                          penalty = penalty_values)
  xy_pred <- multi_predict(xy_fit, seniors_x, type = "numeric",
                           penalty = penalty_values)
  ...
  # single prediction
  f_pred_1 <- multi_predict(f_fit, seniors[1, ], type = "numeric",
                            penalty = penalty_values)
  xy_pred_1 <- multi_predict(xy_fit, seniors_x[1, , drop = FALSE], 
                             type = "numeric",
                             penalty = penalty_values)
